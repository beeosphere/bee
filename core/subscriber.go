...
const (
	beeosCommandPrefixWithoutDot = "$BEEOS"
	beeosCommandPrefix           = "$BEEOS" + "."
	beeosDataPrefix              = "$BEE."
	beeosCommandIndex            = 3
)
...
func (s *subscriber) Topic() string {
	return strings.TrimPrefix(s.topic, beeosDataPrefix)
}
...
func (s *subscriber) subscribe() error { // TODO: Determine queue group subscriptions... take into account...
	...
	subs, err := s.conn.Subscribe(topic, func(msg *nats.Msg) {
		...
		if command := getCommand(msg.Subject); command != "" {
			...
			// Data message
			message := &DataMessage{
				Topic: strings.TrimPrefix(msg.Subject, beeosDataPrefix),
				Data:  msg.Data,
				Route: s.route,
...
func getCommand(topic string) Command {
	...
	if parts[0] == beeosCommandPrefixWithoutDot && len(parts) >= 4 {
